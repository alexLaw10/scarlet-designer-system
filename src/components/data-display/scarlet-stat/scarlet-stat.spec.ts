...
describe('scarlet-stat', () => {
  it('renders label and value', async () => {
    ...
    expect(page.root?.shadowRoot?.querySelector('.scarlet-stat__label')?.textContent?.trim()).toBe('Receita');
    expect(page.root?.shadowRoot?.querySelector('.scarlet-stat__value')?.textContent?.trim()).toBe('R$ 100');
  });
