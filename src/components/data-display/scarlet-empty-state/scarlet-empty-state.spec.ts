...
describe('scarlet-empty-state', () => {
  ...
  it('renders a custom heading and description', async () => {
    ...
    expect(page.root?.shadowRoot?.querySelector('.scarlet-empty-state__heading')?.textContent?.trim()).toBe('Nada por aqui');
    expect(page.root?.shadowRoot?.querySelector('.scarlet-empty-state__description')?.textContent?.trim()).toBe('Crie o primeiro item.');
  });
