...
| `iconOnly`  | `icon-only`  | Makes the button square (width matches its height) instead of sized to its text — for a button whose only content is an icon. Set `ariaLabel` alongside it. | `boolean`                                                                              | `false`     |
