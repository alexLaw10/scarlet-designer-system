...
| `focusable` | `focusable` | Whether this radio is a tab stop. A parent `<scarlet-radio-group>` keeps this `true` on exactly one radio at a time (roving tabindex, per the WAI-ARIA radiogroup pattern) so the group has a single Tab stop and arrow keys move both focus and selection — matching how native radio buttons behave. Standalone (no group), this defaults to `true` so a lone radio is reachable by Tab like any other control. | `boolean`             | `true`      |
