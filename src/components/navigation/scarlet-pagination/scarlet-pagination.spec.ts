...
function pageButtons(page: Awaited<ReturnType<typeof newSpecPage>>) {
  return Array.from(page.root!.shadowRoot!.querySelectorAll('.scarlet-pagination__page')).map(el =>
    el.textContent?.trim()
  );
}
...
describe('scarlet-pagination', () => {
  ...
  it('emits scarletChange with the new page when a page button is clicked', async () => {
    ...
    const buttons = Array.from(
      page.root!.shadowRoot!.querySelectorAll('.scarlet-pagination__page')
  ...
  it('disables the previous button on the first page and the next button on the last page', async () => {
    ...
    const [prevButton, nextButton] = page.root!.shadowRoot!.querySelectorAll(
      '.scarlet-pagination__nav'
  ...
  it('advances one page via the next button', async () => {
    ...
    const nextButton = page.root!.shadowRoot!.querySelectorAll(
      '.scarlet-pagination__nav'
