...
2. Convenções obrigatórias (garantidas pelo ESLint, `@stencil/eslint-config`):
   ...
   - Imports de `src/types`, `src/utils` e `src/utils/*` usam os aliases `@/types`, `@/utils`, `@/utils/form-field` etc. (configurados em `tsconfig.json`) em vez de caminhos relativos `../../` — evita que o caminho quebre se o componente mudar de categoria. Import de outro componente usa `@/components/<categoria>/scarlet-<nome>/scarlet-<nome>`. Já os imports de `.scss` (Sass não entende os aliases do TS) continuam relativos, ex. `@use '../../../tokens/field-mixins' as *;` a partir de `src/components/<categoria>/scarlet-<nome>/`. Use `@use ... as *` (não `@import`, depreciado no Sass) — `as *` deixa os mixins/variáveis sem prefixo, igual usar `@import` antes. Só `src/tokens/field-mixins.scss` foge disso: ele também `@forward`s `typography-mixins`, porque `@use`/`@forward` não são transitivos como `@import` era — quem só dá `@use` em `field-mixins` mas chama um mixin de texto (`scarlet-text-body-sm` etc.) direto no seu `.scss` depende desse `@forward` pra funcionar.
   - Responsividade: os breakpoints (`sm`=640px, `md`=768px, `lg`=1024px, `xl`=1280px, `2xl`=1536px — os mesmos números do `size` de `scarlet-container`) vivem como variáveis Sass em `src/tokens/breakpoints.scss` (`$scarlet-breakpoint-sm` etc.), **não** como custom property — `@media` não resolve `var()`. Para um componente cujo layout deveria mudar por tamanho de tela (como `scarlet-grid`'s `columns`/`columnsMd`/... e `scarlet-stack`'s `direction`/`directionMd`/...), o padrão é: cada breakpoint prop escreve sua própria custom property no host só quando definida, e o `.scss` lê essa cadeia com fallback — `var(--x-md, var(--x-sm, var(--x)))` — para que definir só `md` já valha a partir dali pra cima. Para larguras/alturas que só precisam nunca estourar a viewport (popover, toast), `min()`/`clamp()` já resolve sem precisar de breakpoint — veja `scarlet-toast`/`scarlet-date-picker` como referência.
